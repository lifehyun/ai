'''
x = 'a'
print(ord(x)) #아스키코드를 볼수있는함수 : ord

x = 15
print(bin(x)) #2진수 : 0b
print(hex(x)) #16진수: 0x

x = round(7.65678)  #반올림해주는함수 :round
print(x)

x = max(10,9,-20)   #제일큰수함수 :max
print(x)

x = min([1,5,0,7])  #제일작은수함수를 리스트로 함수로 넣은것
print(x)

x = max((10,8,7,20)) 
print(x)

x = min({10,98,67,10}) #제일작은수를 튜플로 함수로 넣은것
print(x)

x = min({1:"a",0:"b",-1:"c"}) #키의 제일 작은 수 
print(x) 

x = min(['a','A','c']) #유니코드로 제일 작은수 
print(x)

#소수 여부 판별하기 p.299
def is_prime(n) : 
    prime = True
    if n > 1 :                       #만약에 n이 1보다 크다면
        for i in range(2 , n) :
            if n % i == 0 :          #만약에 n을 나눴는데 i랑 같으면
                prime = False
                break
    return prime
